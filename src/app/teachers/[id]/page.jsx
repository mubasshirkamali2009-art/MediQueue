...
const TeacherDetailPage =  () => {
  ...
  async function handleBook(e) {
    ...
    try {
      ...
      setBookingResult({
        success: true,
        message: "Booking confirmed! You'll hear from the tutor soon.",
      });
      setStudentName("");
      setPhone("");
    } catch {
          ...
          <div className="px-5 sm:px-8 pt-6 ">
            ...
            <div className="flex items-center gap-4 mb-6 justify-between">
              ...
              <div>
                ...
                <span
                  className={`inline-block mt-1.5 text-xs font-semibold px-2.5 py-0.5 rounded-full ${modeColor}`}
                >
                  {tutor.teachingMode}
                </span>
              </div>

              <Edit teacher={tutor} onUpdated={(updated) => setTutor(updated)} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-5">
              ...
              <InfoCard
                icon={<Clock size={16} />}
                label="Hours"
                value={`${tutor.timeFrom} – ${tutor.timeTo}`}
              />
              <InfoCard
                icon={<CalendarDays size={16} />}
                label="Days"
                value={tutor.selectedDays?.join(", ")}
              />
              <InfoCard
                icon={<Banknote size={16} />}
                label="Hourly Fee"
                value={`BDT ${tutor.hourlyFee} / hr`}
              />
              <InfoCard
                icon={<CalendarCheck size={16} />}
                label="Session Start"
                value={tutor.startDate}
              />
                ...
                <div>
                  ...
                  <p
                    className={`text-sm font-bold mt-0.5 ${
                      Number(tutor.totalSlots) === 0 ? "text-red-500" : "text-green-600"
                    }`}
                  >
